...
你遇到的错误：
```
404 Publisher Model `projects/your_gcp_project_id/locations/asia-southeast1/publishers/google/models/gemini-2.0-flash-lite-001` was not found
```
...
2. 设置环境变量（在 `.env` 中取消注释）：
   ...
   GRPC_PROXY=http://your-proxy-host:port
   ```
...
3. 或者在 PowerShell 中临时设置：
   ...
   $env:GRPC_PROXY="http://your-proxy-host:port"
   ```
