...
class SessionCategorizedCacheService:
    ...
    async def get_timeline(self, *, session_id: str, category: str) -> list[dict[str, Any]]:
        ...
        if not values:
            ...
            await self._load_timeline_from_sqlite(session_id=session_id, category=category)
        ...
        await self._touch_session(session_id=session_id, now=int(time.time()))
    ...
    async def get_resource_categories(self, *, session_id: str, resource: str) -> dict[str, dict[str, Any]]:
        ...
        if not data:
            self._metrics["sqlite_fallback_loads"] += 1
            await self._load_resource_indexes_from_sqlite(session_id=session_id, resource_key=resource_key)
            data = await redis_client.hgetall(cats_key)

        keys_to_refresh = [
            self._key_resources_zset(session_id),
            self._key_resources_map(session_id),
            cats_key,
        ]
        await self._refresh_ttl(session_id=session_id, resource_key=resource_key, keys=keys_to_refresh)
        await self._touch_session(session_id=session_id, now=now)
    ...
    async def get_resource_category_last(
        ...
        keys_to_refresh = [
            ...
            self._key_resources_map(session_id),
            last_key,
            self._key_resource_cats(session_id, resource_key),
        ]
        await self._refresh_ttl(session_id=session_id, resource_key=resource_key, category=category, keys=keys_to_refresh)
        await self._touch_session(session_id=session_id, now=now)
    ...
    async def list_resources(self, *, session_id: str, limit: int = 100) -> list[str]:
        ...
        if not resource_keys:
            self._metrics["sqlite_fallback_loads"] += 1
            await self._load_resources_from_sqlite(session_id=session_id)
            resource_keys = await redis_client.zrevrange(zset_key, 0, max(0, limit - 1))
        ...
        await self._refresh_ttl(session_id=session_id, keys=[zset_key, map_key])
        await self._touch_session(session_id=session_id, now=now)
